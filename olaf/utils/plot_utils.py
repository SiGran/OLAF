...
def plot_INPS_L(result_df, save_path, header_dict):
    ...
    plt.title(f"Ice Nucleating Particles Number Concentration at {site} for {header_dict['start_time'][:10]}")
    plt.xlabel("Temperature (degC)")
    plt.ylabel("INP Concentration (per L STP)")
    plt.yscale('log')
    plt.ylim(result_df[result_df['lower_CI'] > 0]['lower_CI'].min() * 0.1, result_df['INPS_L'].max() * 3)
    plt.xlim(-30, 0)
    ...
    if "TBS" in site:
        legend_elements = [Patch(facecolor='none', edgecolor='none',label = f"Site: {site}"),
                           Patch(facecolor='none', edgecolor='none',label = f"Date: {header_dict['start_time'][:10]}"),
                           Patch(facecolor='none', edgecolor='none',label = f"Treatment: {header_dict['treatment']}"),
                           Patch(facecolor='none', edgecolor='none',label = f"Lower altitude: {header_dict['lower_altitude']} m agl")                                         ,
                           Patch(facecolor='none', edgecolor='none',label = f"Upper altitude: {header_dict['upper_altitude']} m agl")]
    else:
        legend_elements = [Patch(facecolor='none', edgecolor='none',label = f"Site: {site}"),
                           Patch(facecolor='none', edgecolor='none',label = f"Date: {header_dict['start_time'][:10]}"),
                           Patch(facecolor='none', edgecolor='none',label = f"Treatment: {header_dict['treatment']}")]
    plt.legend(handles=legend_elements, markerscale=0, handlelength=0, handletextpad=0, loc='upper right')
    plt.savefig(save_path)
    #plt.show()


def plot_blank_corrected_vs_pre_corrected_inps(df_corrected, df_original, save_path, plot_header_info):
    """
    Creates individual plots for each sample being blank corrected. Plots show the pre-corrected INP spectrum
    ...
    plt.title(f"Ice Nucleating Particles Number Concentration at {site} for {plot_header_info['start_time'][:10]}")
    handles, labels = plt.gca().get_legend_handles_labels()
    plt.xlabel("Temperature (degC)")
    plt.ylabel("INP Concentration (per L STP)")
    plt.yscale('log')
    plt.ylim(df_corrected[df_corrected['lower_CI'] > 0]['lower_CI'].min() * 0.1, df_corrected['INPS_L'].max() * 3)
    plt.xlim(-30, 0)
    plt.xticks(np.arange(0, -35, -5))
    plt.gca().set_xticks(np.arange(0, -31, -1), minor=True)
    plt.grid(True, linestyle='--', color='gray', linewidth=0.5)
    ...
    if "TBS" in site:
        legend_elements = [Patch(facecolor='none', edgecolor='none', label=f"Site: {site}"),
                           Patch(facecolor='none', edgecolor='none', label=f"Date: {plot_header_info['start_time'][:10]}"),
    ...
    plt.legend(handles=all_handles, markerscale=0, handlelength=0, handletextpad=1, loc='upper right')
    plt.savefig(save_path)

def filter_non_error_signal(df_corrected):
    """
    Removes error signal values from df_corrected to avoid plotting errors due to negative values in a log.
    Parameters
    ----------
    df_corrected
